...
async def send_photo_or_text(chat_id, image_filename, caption, reply_markup=None):
    ...
    try:
        ...
        if image_exists(image_filename):
            ...
            logging.warning(f"Файл {image_filename} не найден в {BASE_DIR}")
    except Exception as e:
...
""")
        ...
        # Проверяем существование файла
        if image_exists("photo10.png"):
            await send_photo_or_text(chat_id, "photo10.png", caption)
        else:
            # Если файла нет, отправляем текстовое описание
            await bot.send_message(chat_id, caption, parse_mode='HTML')
            logging.warning("Файл photo10.png не найден, отправлено текстовое сообщение")
...
⏱️ Время пополнения: 5-15 минут после оплаты
