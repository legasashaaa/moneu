...
def format_bold_text(text):
    """Форматирует весь текст жирным шрифтом и убирает пробелы в начале строк"""
    # Удаляем существующие теги <b> и </b>
    text = text.replace('<b>', '').replace('</b>', '')
    # Разбиваем на строки и удаляем пробелы в начале каждой строки
    lines = text.split('\n')
    stripped_lines = [line.lstrip() for line in lines]
    # Оборачиваем весь текст в теги <b>
    return f"<b>{''.join(stripped_lines)}</b>"
