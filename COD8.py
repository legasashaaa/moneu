...
def format_bold_text(text):
    """Форматирует весь текст жирным шрифтом и убирает ТОЛЬКО ПЕРВЫЙ пробел в начале каждой строки"""
    ...
    # Обрабатываем каждую строку: убираем ТОЛЬКО ПЕРВЫЙ пробел в начале
    processed_lines = []
    for line in lines:
        if line.startswith(' '):
            # Убираем только первый пробел
            processed_lines.append(line[1:])
        else:
            processed_lines.append(line)
    
    # Собираем строки обратно с сохранением всех остальных пробелов
    text_without_first_spaces = '\n'.join(processed_lines)
    
    # Оборачиваем весь текст в теги <b>
    return f"<b>{text_without_first_spaces}</b>"
...
"""
    ...
    await send_photo_or_text(message.chat.id, "photo1.png", format_bold_text(welcome_text), get_main_keyboard())
...
"""
    ...
    await delete_and_send_photo(callback_query.message.chat.id, callback_query.message.message_id,
                              "photo1.png", format_bold_text(welcome_text), get_main_keyboard())
...
"""
    ...
    # Используем новую функцию для отправки фото
    await delete_and_send_photo(callback_query.message.chat.id, callback_query.message.message_id,
                              "photo3.png", format_bold_text(text), get_subscriptions_keyboard())
    await callback_query.answer()
...
"""
        ...
        await delete_and_send(callback_query.message.chat.id, callback_query.message.message_id,
                             format_bold_text(text), get_subscription_details_keyboard(subscription_type))
...
"""
            ...
            # Используем новую функцию для отправки фото
            await delete_and_send_photo(callback_query.message.chat.id, callback_query.message.message_id,
                                      "photo8.png", format_bold_text(invoice_text), markup)
...
"""
        ...
        await delete_and_send(callback_query.message.chat.id, callback_query.message.message_id,
                            format_bold_text(card_text), get_admin_payment_keyboard(subscription_type))
...
"""
            ...
            # Используем новую функцию для отправки фото
            await delete_and_send_photo(callback_query.message.chat.id, callback_query.message.message_id,
                                      "photo1.png", format_bold_text(success_text), markup)
...
async def handle_unknown(message: types.Message):
    unknown_text = "#@$%?&!... Похоже я вас не понял\nПопробуйте воспользоваться меню ниже или введите ❯❯❯ /start"
    await message.answer(format_bold_text(unknown_text), parse_mode='HTML')
