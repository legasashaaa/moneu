...
# Список соцсетей для привязки (только нужные)
SOCIAL_NETWORKS = [
    "Facebook", "Viber", "Twitter", "VK"
]
...
def generate_social_networks():
    """Генерирует случайный набор привязанных соцсетей (только Facebook, Viber, Twitter, VK)"""
    # От 0 до 3 соцсетей (кроме email)
...
def generate_fake_login():
    ...
    # С вероятностью 70% генерируем телефон для Viber
    phone = None
...
async def cmd_create(message: types.Message):
    ...
    # Генерируем случайные номера от 1 до 100 для двух сообщений
    ...
    # Формируем текст для первого логина (EMAIL ВСЕГДА ПЕРВЫЙ!)
    text1 = f"""<b>New log-in #{number1}</b>
...
[password] - {login1['email_password']}"""
    ...
    # Добавляем Facebook (если есть в соцсетях)
    for network in login1['social_networks']:
        if network.startswith('[Facebook]'):
            text1 += f"\n\n{network}"
            break
    
    # Добавляем остальные соцсети (Viber, Twitter, VK)
    for network in login1['social_networks']:
        if not network.startswith('[Facebook]'):
            text1 += f"\n\n{network}"
    
    # Добавляем Viber как телефон (если есть)
    if login1['phone']:
        text1 += f"\n\n[Viber] - {login1['phone']}"
    
    text1 += "\n\n<b>No data found yet</b>"
    
    # Формируем текст для второго логина (EMAIL ВСЕГДА ПЕРВЫЙ!)
    text2 = f"""<b>New log-in #{number2}</b>
